...
async function helper(productData, result) {
  const { name, short_description, long_description, price, available_stock_count, img_link, category_id, stock_count, seller_id } = productData;
  try {
    console.log('Thêm sản phẩm vào danh mục...');
    await query(`INSERT INTO product_categories (product_id, category_id) VALUES ($1, $2)`, [result.rows[0].id, category_id]);
  } catch (error) {
    console.error("Lỗi khi tạo sản phẩm:", error);
    throw error; // Ném lỗi lên để xử lý ở tầng trên
  }
  try {
    console.log('Thêm sản phẩm vào danh sách sản phẩm của người bán...');
    //await query(`INSERT INTO seller_products (seller_id, product_id) VALUES ($1, $2)`, [seller_id, result.rows[0].id]);
  } catch (error) {
    console.error("Lỗi khi tạo sản phẩm:", error);
    throw error; // Ném lỗi lên để xử lý ở tầng trên
  }
...
async function createProduct(productData) {
  ...
  const final_link = (!img_link || img_link === '') ? 'https://img.upanh.tv/2024/05/30/San-phm-nay-khong-co-hinh-anh.png' : img_link;
  try {
    console.log('Tạo sản phẩm mới...');
    const result = await query(
      `INSERT INTO products (name, short_description, long_description, price, img_link, available_stock_count, stock_count, category_id, seller_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`,
      [name, short_description, long_description, price, final_link, available_stock_count, available_stock_count, category_id, seller_id]
    );
    helper(productData, result);

    return result.rows[0]; // Trả về sản phẩm mới được tạo
  } catch (error) {
    console.error("Lỗi khi tạo sản phẩm:", error);
    throw error; // Ném lỗi lên để xử lý ở tầng trên
  }
